...
## 3. Test Smells and Anti-Patterns
...
An **ice-cream cone** anti-pattern inverts the pyramid: lots of manual/E2E tests at the top,
very few unit tests at the bottom. This makes the suite slow, brittle, and expensive to maintain.

---

## What You Learned

- Testing pyramid: unit > integration > E2E — favour many fast unit tests.
- JUnit 5 lifecycle: `@BeforeEach`, `@AfterEach`, `@TestFactory` for dynamic tests.
- AssertJ for fluent, readable assertions that scale beyond JUnit defaults.
- Mockito for test doubles — `when().thenReturn()`, `verify()`, `@MockBean` in Spring tests.
- TDD cycle: Red → Green → Refactor — write the test first, never skip the refactor step.
- Spring Boot test slices: `@WebMvcTest`, `@DataJpaTest`, `@JsonTest` — test only what you need.
- JaCoCo for coverage — track trends, don't chase 100%.
- Integration testing with Testcontainers: real Kafka, real Postgres, real confidence.
- StepVerifier for reactive stream assertions: `expectNext`, `expectComplete`, `verify()`.
- Anti-patterns: ice-cream cone, testing implementation details, not testing edge cases.
