...
When `OrderService` needs to notify `CustomerService` that an order was placed, you *could* inject `CustomerService` directly. But if `CustomerService` also calls `OrderService`, you get a **circular dependency** — Spring won't start. Application events solve this: the publisher doesn't know who listens.
...
A record — immutable, trivial to construct. Events should carry only what the listener needs, not the entire entity.
