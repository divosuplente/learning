...
That's 4+ round trips. Each network hop adds 50 to 200 ms of latency. On a slow connection, the dashboard is unusable.
...
**GraphQL** is a query language for your API. Instead of the server deciding the response shape, the **client specifies exactly what it wants** in the request body, and the server returns exactly that, nothing more.
...
The client can request related data in a single query, with no extra round trips:
...
<p><strong>Correct answer:</strong> Each new client subset requires a new endpoint, causing endpoint sprawl with fixed shapes that never match every client's needs</p>
</details>
