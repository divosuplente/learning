...
Each row in `@CsvSource` is one test execution. If the third row fails, the other three still pass, so you get a precise signal about which input broke the logic.
...
**Common trap:** `assertThat(new BigDecimal("1.0")).isEqualTo(new BigDecimal("1.00"))` fails because `isEqualTo` checks `.equals()`, which considers scale. Use `isEqualByComparingTo` for numeric comparison.
...
-   **Inject dependencies, don't create them.** A service that `new`s its own `OrderRepository` cannot be tested with a fake. Accept it via constructor, and the test supplies a mock or stub.
...
<p><strong>Correct answer:</strong> It chains further assertions on the exception, like .hasMessageContaining("999"), which assertThrows cannot do fluently</p>
</details>
